...
static void findAspTags (void)
{
	...
	while ((line = fileReadLine ()) != NULL)
	{
		...
		while (*cp != '\0')
		{
			...
			else if (strncasecmp ((const char*) cp, "end", (size_tee) 3)== 0)
			{
				cp += 3;
				if (isspace ((int)*cp))
				{
					...
					if (strncasecmp ((const char*) cp, "function", (size_tee) 8) == 0)
					{
						cp+=8;
						break;
					}

					else if (strncasecmp ((const char*) cp, "sub", (size_tee) 3) == 0)
					{
						cp+=3;
						break;
					}
			...
			else if (strncasecmp ((const char*) cp, "exit", (size_tee) 4)==0)
			{
				cp += 4;
				if (isspace ((int) *cp))
				{
			...
			/* class member? */
			else if (strncasecmp ((const char*) cp, "public", (size_tee) 6) == 0)
			{
				cp += 6;
				if (isspace ((int) *cp))
				{
			...
			else if (strncasecmp ((const char*) cp, "private", (size_tee) 7) == 0)
			{
				cp += 7;
				if (isspace ((int) *cp))
				{
			...
			/* function? */
			else if (strncasecmp ((const char*) cp, "function", (size_tee) 8) == 0)
			{
				cp += 8;
			...
			/* dim variable? */
			else if (strncasecmp ((const char*) cp, "dim", (size_tee) 3) == 0)
			{
				cp += 3;
				if (isspace ((int) *cp))
				{
			...
			/* class declaration? */
			else if (strncasecmp ((const char*) cp, "class", (size_tee) 5) == 0)
			{
				cp += 5;
				if (isspace ((int) *cp))
				{
			...
			/* const declaration? */
			else if (strncasecmp ((const char*) cp, "const", (size_tee) 5) == 0)
			{
				cp += 5;
				if (isspace ((int) *cp))
				{
