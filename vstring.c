...
*   DATA DEFINITIONS
*/
static const size_tee vStringInitialSize = 32;

/*
*   FUNCTION DEFINITIONS
*/

static void vStringResize (vString *const string, const size_tee newSize)
{
...
extern boolean vStringAutoResize (vString *const string)
{
	...
	{
		const size_tee newSize = string->size * 2;
...
extern void vStringNCatS (
		vString *const string, const char *const s, const size_tee length)
{
	const char *p = s;
	size_tee remain = length;
...
{
	const size_tee final = string->length - 1;
...
{
	const size_tee length = src->length;
