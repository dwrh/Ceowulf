...
static int strnuppercmp (const char *s1, const char *s2, size_tee n)
{
	int result;
	do
	{
...
static int readTagLineRaw (tagFile *const file)
{
	...
	do
	{
		...
		*pLastChar = '\0';
		...
		{
			size_tee i = strlen (file->line.buffer);
			while (i > 0  &&
...
{
	fpos_t startOfLine;
	const size_tee prefixLength = strlen (PseudoTagPrefix);
	if (info != NULL)
	{
		info->file.format     = 1;
...
{
	tagFile *result = (tagFile*) calloc ((size_tee) 1, sizeof (tagFile));
	if (result != NULL)
	{
