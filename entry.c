...
static void rememberMaxLengths (const size_tee nameLength, const size_tee lineLength)
{
		...
		const char *const pattern)
{
	...
	rememberMaxLengths (strlen (tagName), (size_tee) length);
}
...
static boolean isCtagsLine (const char *const line)
{
	...
	const size_tee fieldLength = strlen (line) + 1;
	...
	{
#define field(x)		(fields + ((size_tee) (x) * fieldLength))
...
extern void copyBytes (FILE* const fromFp, FILE* const toFp, const long size)
{
	...
	do
	{
		...
		numRead = fread (buffer, (size_tee) 1, (size_tee) toRead, fromFp);
		if (fwrite (buffer, (size_tee)1, (size_tee)numRead, toFp) < (size_tee)numRead)
 ...
 */
static size_tee writeSourceLine (FILE *const fp, const char *const line)
{
	size_tee length = 0;
	const char *p;
 ...
 */
static size_tee writeCompactSourceLine (FILE *const fp, const char *const line)
{
...
extern void makeTagEntry (const tagEntryInfo *const tag)
{
	...
	else
	{
		...
		++TagFile.numTags.added;
		rememberMaxLengths (strlen (tag->name), (size_tee) length);
