...
static boolean createTagsForWildcardEntry (
		const char *const pattern, const size_tee dirLength,
...
{
	boolean resize = FALSE;
	const size_tee dirLength = baseFilename (pattern) - pattern;
...
static boolean createTagsForAmigaWildcard (const char *const pattern)
{
	...
	struct AnchorPath *const anchor =
			(struct AnchorPath *) eMalloc ((size_tee) ANCHOR_SIZE);
	LONG result;

	memset (anchor, 0, (size_tee) ANCHOR_SIZE);
