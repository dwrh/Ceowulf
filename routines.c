...
{
	const size_tee length = strlen (substr);
...
extern void setCurrentDirectory (void)
{
...
#endif
	if (CurrentDirectory == NULL)
		CurrentDirectory = xMalloc ((size_tee) (PATH_MAX + 1), char);
#ifdef AMIGA
	...
	if (buf == NULL)
		perror ("");
#endif
	if (CurrentDirectory [strlen (CurrentDirectory) - (size_tee) 1] !=
			PATH_SEPARATOR)
	{
...
extern boolean isRecursiveLink (const char* const dirName)
{
	...
	if (status->isSymbolicLink)
	{
		...
		while (! result  &&  strlen (path) > (size_tee) 1)
		{
