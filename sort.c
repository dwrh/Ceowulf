...
extern void externalSortTags (const boolean toStdout)
{
	...
	const size_tee length = 4 + strlen (sortOrder1) + strlen (sortOrder2) +
...
static void writeSortedTags (
		char **const table, const size_tee numTags, const boolean toStdout)
{
	FILE *fp;
	size_tee i;
...
extern void internalSortTags (const boolean toStdout)
{
	 ...
	 */
	size_tee numTags = TagFile.numTags.added + TagFile.numTags.prev;
	const size_tee tableSize = numTags * sizeof (char *);
	char **const table = (char **) malloc (tableSize);  /* line pointers */
	DebugStatement ( size_tee mallocSize = tableSize; )  /* cumulative total */
	...
	for (i = 0  ;  i < numTags  &&  ! feof (fp)  ;  )
	{
		...
		{
			const size_tee stringSize = strlen (line) + 1;
