...
static void parseLongOption (cookedArgs *const args, const char *item)
{
	...
	{
		const size_tee length = equal - item;
	...
	if (separator != NULL)
	{
		...
		*separator = '\0';
		...
		if (language != LANG_IGNORE)
		{
			...
			if ((size_tee) (p - list) == strlen (deflt) &&
		...
		const char **const replacement)
{
	...
	{
		const size_tee nameLen = strlen (name);
		unsigned int i;
		...
		for (i = 0  ;  i < stringListCount (Option.ignore)  ;  ++i)
		{
			...
			{
				const size_tee tokenLen = vStringLength (token);
