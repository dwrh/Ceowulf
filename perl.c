...
static boolean isPodWord (const char *word)
{
	...
	if (isalpha (*word))
	{
		...
		const size_tee count = sizeof (pods) / sizeof (pods [0]);
		const char *white = strpbrk (word, " \t");
		const size_tee len = (white!=NULL) ? (size_tee)(white-word) : strlen (word);
...
static void findPerlTags (void)
{
	...
	while ((line = fileReadLine ()) != NULL)
	{
		...
		if (skipPodDoc)
		{
			if (strncmp ((const char*) line, "=cut", (size_tee) 4) == 0)
				skipPodDoc = FALSE;
			continue;
		}
		else if (line [0] == '=')
		...
		if (strncmp((const char*) cp, "sub", (size_tee) 3) == 0)
		{
		...
		else if (strncmp((const char*) cp, "use", (size_tee) 3) == 0)
		{
			cp += 3;
			if (!isspace(*cp))
				continue;
			while (*cp && isspace (*cp))
				++cp;
			if (strncmp((const char*) cp, "constant", (size_tee) 8) != 0)
				continue;
			cp += 8;
			kind = K_CONSTANT;
			spaceRequired = TRUE;
		...
		else if (strncmp((const char*) cp, "package", (size_tee) 7) == 0)
		{
		...
		else if (strncmp((const char*) cp, "format", (size_tee) 6) == 0)
		{
			cp += 6;
			kind = K_FORMAT;
			spaceRequired = TRUE;
